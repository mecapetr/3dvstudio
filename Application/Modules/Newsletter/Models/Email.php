...
class Newsletter_Models_Email extends Models_DbTable
{
    ...
    public function getEmailsByUserID($userID,$limit){
    	...
    	$sql = "
        		SELECT E.emailID,E.title,E.date 
        			FROM user C JOIN user_email CE ON(C.userID = CE.userID AND C.userID = ?)
    ...
    public function getCountEmailsByUserID($userID){
    	$sql = "
        		SELECT COUNT(*) 
        			FROM user C 	
        			JOIN customer_email CE ON(C.userID = CE.userID AND C.userID = ?)
        			JOIN email E ON (CE.emailID = E.emailID)
        		";
